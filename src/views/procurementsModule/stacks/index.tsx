...
import ModalDeleteStack from './ModalDeleteStack'
import { MyLoading, NotFound } from '../../../Helpers/custom';
import ModalAddStack from './ModalAddStack';
import { C5_DeleteStackAction, C5_getSearchStacksAction, C5_getStacksAction } from '../../../redux/actions/ProcurementsModule/stacks';
...
interface EnhancedTableToolbarProps {
  numSelected: number;
  searchStackValue:any;
  showModal:()=>void;
}

const EnhancedTableToolbar = (props: EnhancedTableToolbarProps) => {
  const classes = useToolbarStyles();
  const { numSelected,searchStackValue,showModal } = props;
      ...
      <nav className="navbar navbar-expand navbar-white navbar-light">
          <ul className="navbar-nav ml">
            ...
            <li className="nav-item">
                <TextField fullWidth label="Search Stacks"  onKeyUp={searchStackValue}  placeholder="Search Stacks" size='small' />
            </li>
            {/* user profile */}
          ...
          <ul className="navbar-nav ml-auto">
            <li className="nav-item">
              {CAN(UserAction.Create,EntityUSERMODAbilty.STACKS) && (
                          <Tooltip title="Add Stack" aria-label="add" onClick={()=>showModal()}>
...
let sStack:SearchStack={value:''};
const  StacksView =(props:any)=>{
  ...
  const [openDeleteStackModal, setopenDeleteStackModal] = React.useState(false);
  ...
  const _closeDeleteStackModal=()=>setopenDeleteStackModal(false)
...
const _showEditStackModal=(user:any)=>()=>{
  setStack(user)
  setisAddMode(false)
  setopenAddModal(true)
}
...
//searchStacksAction
const _searchStackValue=(e:any)=>{
  let value=e.target.value.trim();
  sStack={...sStack,value:value}
  ...
  setMyLoading(false)
  props.searchStacksAction(sStack,onSuccess,onError);
}

const _showDeleteStackModal=(stack:any)=>()=>{
  setStack(stack)
  let childDelete= "Warning!! This Stack "+ stack.code + " will be definitevely deleted";
  setChildrenDelete(childDelete)
  setopenDeleteStackModal(true)
}


const  _handledeleteStack=()=> {
  let {id,serial,code}=stack;
  const onDeleteStackSuccess=(res:any)=>{
      const onSuccesGet=(res:any)=>{
        setButtonLoading(false)
        setopenDeleteStackModal(false)
      }
      const onErrorGet=(res:any)=>{console.log(res)}
      let message=code+" deleted successfully";
      toast.success(message,{position:toast.POSITION.BOTTOM_CENTER});
        props.getStackAction(onSuccesGet,onErrorGet)
  ...
  /////end my custom function /////////////
    return (
         <>
                      ...
                      <div className={classes.root}>
                          <Paper className={classes.paper}>
                            <EnhancedTableToolbar numSelected={selected.length}
                             searchStackValue={_searchStackValue}
                              showModal={_showAddStackModal()}/>
                                          ...
                                          .map((row:any, index:any) => {
                                            ...
                                            return (
                                              ...
                                              >
                                                ...
                                                {CAN(UserAction.Update,EntityUSERMODAbilty.STACKS) && (
                                                  row.createdBy==authUser.id || role.name==USERROLES.PRMTADMIN || role.name==USERROLES.SUPERADMIN?
                                                  <Tooltip title={"Edit "+row.code}><IconButton onClick={_showEditStackModal(row)} color="success"><EditIcon/></IconButton></Tooltip>    
                                                  :'')} 
                                                {CAN(UserAction.Delete,EntityUSERMODAbilty.STACKS) && (
                                                    row.createdBy==authUser.id || role.name==USERROLES.PRMTADMIN || role.name==USERROLES.SUPERADMIN?
                                                  <Tooltip title={"Delete "+row.code}><IconButton onClick={_showDeleteStackModal(row)} color="error"> <DeleteIcon/></IconButton></Tooltip>
                                                 :'')}
                                                </TableCell>
            ...
            {openAddModal?
              <ModalAddStack openStackModal={openAddModal} closeAddModal={_closeAddModal} stack={isAddMode ? '':stack} isAddMode={isAddMode}/>
            :''}

             {openDeleteStackModal?
                <MyConfirmModal  title="Delete Stack?"  open={openDeleteStackModal}  setOpen={setopenDeleteStackModal} 
                 onConfirm={_handledeleteStack} buttonLoading={buttonLoading}>
                   {childrenDelete}
                </MyConfirmModal>   
              :''}
    ...
    const mapStateToProps = (state:StateProps) => {
        ...
        return { getstacks,getuserprofile}
    };
    const mapDispatchToProps = (dispatch:any)=>{
        return {
            getStackAction: (onSuccess:any,onError:any)=> dispatch(C5_getStacksAction(onSuccess,onError)),
            searchStacksAction:(value:SearchStack,onSuccess:any,onError:any)=> dispatch(C5_getSearchStacksAction(value,onSuccess,onError)),
            deleteStackAction: (id:string,onSuccess:any,onError:any)=> dispatch(C5_DeleteStackAction(id,onSuccess,onError)),
        }
    }

    export default connect(mapStateToProps,mapDispatchToProps)(StacksView);